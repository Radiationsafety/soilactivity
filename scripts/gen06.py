#!/usr/bin/env python3
"""Generate example06_interpretation.ipynb via json.dump()."""

import json
import os

NOTEBOOK_PATH = "/home/z/my-project/soilactivity/examples/example06_interpretation.ipynb"
...
def build_notebook():
    ...
    cells.append(md([
        ...
        "\n",
        "This notebook explores the **spatial_interpolation** module of *soilactivity*,",
        "focusing on two critical questions for environmental monitoring campaigns:\n",
        "\n",
        "> 1. **Which interpolation method should I use?** — Automated cross-validation",
        ">    comparison across multiple backends (RBF, Delaunay, IDW, Barnes, Cressman, etc.).\n",
        "> 2. **Which measurements matter most?** — Leave-one-out sensitivity analysis",
        ">    that quantifies each measurement point's influence on the interpolated field.\n",
        "\n",
        "The approach is inspired by **bssunfold**'s ``unfold_interpret`` module for",
        "unfold sensitivity diagnostics and the **pyoptexplain** pattern for explaining",
        "black-box predictions through perturbation-based analysis.\n",
        "\n",
        "We use synthetic Ambient Dose Equivalent Rate (ADER) data with three hot spots",
        "to demonstrate method selection, critical point identification, and the impact",
        "of sparse versus dense measurement networks.\n",
    ]))
    ...
    # ===== Section 3: Available Methods =====
    cells.append(md([
        "## Available Interpolation Methods\n",
        "\n",
        "The ``spatial_interpolation`` module ships with 14 backends ranging from",
        "fast nearest-neighbour lookups to Gaussian-Process regression with built-in",
        "uncertainty quantification. The ``AVAILABLE_METHODS`` catalogue below lists",
        "every method and its description.\n",
    ]))
    ...
    # ===== Section 4: Synthetic ADER Data =====
    cells.append(md([
        "## Synthetic ADER Data\n",
        "\n",
        "We generate 100 measurement points randomly placed in a 1 km \u00d7 1 km area",
        "with three Gaussian hot spots of varying intensity, plus a uniform background",
        "of 0.1 \u00b5Sv/h. Lognormal noise simulates realistic measurement uncertainty.\n",
        "\n",
    ...
    # ===== Section 5: Interpolation Comparison =====
    cells.append(md([
        "## Interpolation Comparison\n",
        "\n",
        "The ``InterpolationAutoSelector`` runs k-fold cross-validation (k=5) across",
        "six candidate methods and ranks them by RMSE.  We then visualise each", 
        "method's interpolated field side-by-side.\n",
    ]))
    ...
    cells.append(md([
        ...
        "\n",
        "Inspired by **bssunfold**'s ``unfold_interpret`` and the **pyoptexplain**",
        "framework, we use ``MeasurementSensitivityAnalyzer`` to perform leave-one-out",
        "analysis. Removing each point in turn and measuring the change in the interpolated",
        "field reveals which measurements are most *influential*.\n",
    ]))

    # Sensitivity code cell 1: LOO analysis + ranking + critical points
    cells.append(code([
    ...
    # Sensitivity code cell 2: 1x3 subplot
    cells.append(code([
    ...
    # Sensitivity code cell 3: Interpretation printout
    cells.append(code([
        "print('=' * 60)\n",
    ...
    cells.append(md([
        ...
        "\n",
        "What happens when we only have 10 measurements instead of 100?  We compare",
        "the full-data interpolation (100 pts) against a sparse sub-sample (10 random", 
        "pts) and visualise the difference.\n",
    ]))
    ...
    cells.append(md([
        ...
        "### Method Selection\n",
        "\n",
        "- The ``InterpolationAutoSelector`` automatically identifies the best method",
        "  via cross-validation. For data with sharp hot spots, smooth RBF methods",
        "  (e.g. ``rbf_tps``) typically outperform nearest-neighbour and linear approaches.\n",
        "- Barnes and Cressman methods provide good middle-ground performance with",
        "  physically motivated weighting functions.\n",
        "\n",
        "### Critical Measurements\n",
        "\n",
        "- Leave-one-out sensitivity analysis reveals that a small subset of measurements", 
        "  (often those nearest to hot-spot peaks) dominate the interpolated field.\n",
        "- The top 3 influential points can account for a large fraction of total influence,",
        "  meaning their removal would significantly degrade interpolation quality.\n",
        "- Identifying critical points guides survey planning: ensure these locations are",
        "  measured with high precision and re-sampled if quality checks fail.\n",
        "\n",
        "### Sparse Interpolation\n",
        "\n",
        "- Reducing the network from 100 to 10 random points causes substantial",
        "  degradation in hot-spot reconstruction, with RMSE differences reflecting",
        "  lost spatial detail.\n",
        "- A *targeted* sparse network (using sensitivity analysis to choose points)",
        "  would significantly outperform a random sub-sample.\n",
        "- This underscores the value of adaptive sampling strategies in environmental",
        "  radiation monitoring campaigns.\n",
    ]))
...
def main():
    ...
    print("Wrote {} ({} cells)".format(NOTEBOOK_PATH, len(cells)))
